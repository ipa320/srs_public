...
srs::COctoMapPlugin::COctoMapPlugin(const std::string & name) :
	srs::CServerPluginBase(name), filecounter(0) {
	//
	setDefaults();
...
void srs::COctoMapPlugin::init(ros::NodeHandle & node_handle) {
	...
	*m_ciSubscriber = node_handle.subscribe(m_camera_info_topic, 10,
			&srs::COctoMapPlugin::cameraInfoCB, this);
 ...
 */
void srs::COctoMapPlugin::insertScan(const tf::Point & sensorOriginTf,
		const tPointCloud & ground, const tPointCloud & nonground) {
...
void srs::COctoMapPlugin::filterGroundPlane(const tPointCloud & pc,
		tPointCloud & ground, tPointCloud & nonground) const {
...
void srs::COctoMapPlugin::crawl(const ros::Time & currentTime) {
	...
	// Crawl through nodes
	for (srs::tButServerOcTree::leaf_iterator
			it = m_data->octree.begin_leafs(), end = m_data->octree.end_leafs(); it
			!= end; ++it) {
...
/// Handle node
void srs::COctoMapPlugin::handleNode(tButServerOcTree::iterator & it,
		const SMapParameters & mp) {
	m_sigOnNode(it, mp);
}

/// Handle free node
void srs::COctoMapPlugin::handleFreeNode(tButServerOcTree::iterator & it,
		const SMapParameters & mp) {
	m_sigOnFreeNode(it, mp);
}

/// Handle occupied node
void srs::COctoMapPlugin::handleOccupiedNode(tButServerOcTree::iterator & it,
		const SMapParameters & mp) {
 ...
 *
 */
bool srs::COctoMapPlugin::resetOctomapCB(std_srvs::Empty::Request& request,
...
void srs::COctoMapPlugin::cameraInfoCB(
 ...
 */
void srs::COctoMapPlugin::degradeOutdatedRaycasting(
	...
	m_tfListener.lookupTransform(sensor_header.frame_id,
			m_mapParameters.frameId, sensor_header.stamp, trans);
	...
	for (tButServerOcTree::leaf_iterator it = m_data->octree.begin_leafs(),
		...
		if (m_data->octree.isNodeOccupied(*it)) {
			...
			for (key[2] = nKey[2] - 1; !neighborFound && key[2] <= nKey[2] + 1; ++key[2]) {
				for (key[1] = nKey[1] - 1; !neighborFound && key[1] <= nKey[1]
						+ 1; ++key[1]) {
 ...
 */
void srs::COctoMapPlugin::computeBBX(const std_msgs::Header& sensor_header,
		octomap::point3d& bbx_min, octomap::point3d& bbx_max) {
	...
	for (int i = 0; i < 8; i++) {
		...
		m_tfListener.transformPoint(m_mapParameters.frameId, stamped_in,
				stamped_out);
 ...
 */
bool srs::COctoMapPlugin::isOccludedMap(const octomap::point3d& sensor_origin,
		const octomap::point3d& p) const {
...
octomap::point3d srs::COctoMapPlugin::getSensorOrigin(
		const std_msgs::Header& sensor_header) {
	geometry_msgs::PointStamped stamped_in;
	geometry_msgs::PointStamped stamped_out;
	stamped_in.header = sensor_header;
