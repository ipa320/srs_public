		...
		gdatabase::GetWorkspaceOnMap::Response &res )
{
	...
	string query_String("SELECT object_id,object_class_id FROM object JOIN semantic ON object.id=semantic.parent_object_id WHERE map_id=");
	ostringstream map;
	map<<req.mapID;
...
{
	ros::init(argc, argv, "get_workspace_map_server");
	ros::NodeHandle wm;

	ros::ServiceServer workspaceonmap = wm.advertiseService("GetWorkspaceOnMap", getWorkspaceOnMap);
