...
echo "Waiting for a command on /DM_UI/interface_cmd... "
 ...
 if [ $(rostopic echo -n 1 /DM_UI/interface_cmd | grep command| cut -b 10-17) = "continue" ]; then
    `rostopic pub -1 /DM_UI/interface_cmd_response srs_msgs/UI_DMresp  "continue" "sssss" 5 > /dev/null 2>&1` 
 else 
    `rostopic pub -1 /DM_UI/interface_cmd_response srs_msgs/UI_DMresp "move" "home" 6 > /dev/null 2>&1`
 fi
