...
#define POINTCLOUD_CENTERS_PUBLISHER_NAME std::string("butsvr_rviz_visible_pointcloud_centers")
#define CAMERA_POSITION_TOPIC_NAME std::string("/rviz_camera_position")

/**
 * Constructor
 */
srs::CLimitedPointCloudPlugin::CLimitedPointCloudPlugin( const std::string & name )
: srs::CPointCloudPlugin( name, false )
 ...
 */
srs::CLimitedPointCloudPlugin::~CLimitedPointCloudPlugin()
{
	if (spin_thread_.get())
	{
 ...
 */
void srs::CLimitedPointCloudPlugin::spinThread()
{
...
void srs::CLimitedPointCloudPlugin::onFrameStart( const SMapParameters & par )
{
    // Reset counters
 ...
 */
void srs::CLimitedPointCloudPlugin::onCameraPositionChangedCB(const srs_env_model_msgs::RVIZCameraPosition::ConstPtr& cameraPosition)
{
...
void srs::CLimitedPointCloudPlugin::onPublish(const ros::Time & timestamp)
{
//    PERROR( "Visible: " << m_countVisible << ", hidden: " << m_countHidden << ", min: " << min << ", max: " << max );
//    PERROR( "Num of points: " << m_data->size() );
    srs::CPointCloudPlugin::onPublish( timestamp );
}
